...
| -------------- | -------------------------------------------------------------- | --------------------------------------- |
| `root`         | <pre>= prop<br/>\| '[' '^'? '-'? s seq* [\\\$ \\-]? ']' <br/>\| var</pre> | \\p{x=y},<br/>[abc],<br/>$myset                     |
| `seq`          | <pre>= root (s [\\&\\-] s root)* s<br/>\| range s</pre>        | [abc]-[cde], a                          |
| `range`        | <pre>= element ('-' element)?       | a, a-c, \{abc}, {ax}-{bz}                          |
...
| `charInString` | <pre>= [^ \\\\ \\} [:Pat_WS:]]<br/>\| '\\' quoted</pre> | a, b, c, \\n, \{, $ <br/> *note: only quoted items that resolve to one code point are allowed* |
| `quoted`       | <pre>= 'u' (hex{4} \| bracketedHex)<br/>\| 'x' (hex{2} \| bracketedHex)<br/>\| 'U00' ('0' hex{5} \| '10' hex{4})<br/>\| 'N{' propName '}'<br/>\| [[\u0000-\U00010FFFF]-[uxUN]]</pre> | n, U0000FFFE, \{, \$, \] <br/> _note: lengths are exact_ |
| `charName`     | <pre>= s [A-Za-z0-9] [-A-Za-z0-9_\x20]* s</pre>                | TIBETAN LETTER -A                       |
| `bracketedHex` | <pre>= '{' s hexCodePoint (s hexCodePoint)* s '}'</pre>        | \{61 2019 62}  <br/> *note: the whitespace between hexCodePoints is required here*                         |
| `hexCodePoint` | <pre>= hex{1,5} \| '10' hex{4}</pre>                           |                                         |
| `hex`          | <pre>= [0-9A-Fa-f]</pre>                                       |                                         |
| `var`          | <pre>= '$' [:ID_Start:] [:ID_Continue:]*</pre>                 | $a, $elt5                    |
| `s`            | <pre>= [:Pattern_White_Space:]*</pre>                          | optional whitespace                     |

<!-- EBNF in playground format:
root = prop
| '[' '^'? '-'? s seq* [\$ \-]? ']' 
| var ;
...
element = char | string | var ;

string = '{' (s charInString)* s '}' ;
...
quoted = 'u' (hex{4} | bracketedHex)
| 'x' (hex{2} | bracketedHex)
| 'U00' ('0' hex{5} | '10' hex{4})
| 'N{' propName '}'
| [[\u0000-\U00010FFFF]-[uxUN]] ;

charName = s [A-Za-z0-9] [-A-Za-z0-9_\x20]* s ;
...
var = '$' [:ID_Start:] [:ID_Continue:]* ;

s = [:Pattern_White_Space:]* ; 
-->
...
charInString = [^ \\ } [:Pat_WS:]] | \\ quoted ;

quoted = (u (hex{4} | bracketedHex)) | (x (hex{2} | bracketedHex)) | (U00 (0 hex{5}) | (10 hex{4})) | (N\{ propName \}) | ([[\u0000-\U00010FFFF]-[uxUN]]) ;
...
<!-- limited version in syntax supported by https://bnfplayground.pauliankline.com/
<root> ::= <prop> | ("[" "^"? "-"? <s> <seq>* ("$" | "-")? "]") | <var> 
...
<prop> ::= ("\\" ("p" | "P") "{" <propName> (("!=" | "=") <s> <value1>+)? "}") | ("[:" "^"? <propName> (("!=" | "=") <s> <value2>+)? ":]") 

<propName> ::= <s> ([A-Z] | [a-z] | [0-9]) ([A-Z] | [a-z] | [0-9] | "_" | " ")* <s> 

/* limitation: not doing [^}] */
<value1> ::= [a-z] | ("\\" <quoted>) 
...
/* limitation: not doing [^ \^ \& \- \[ \[ \] \\ \{ \$ [:Pat_WS:]] */
<char> ::= [a-z] | "\\" <quoted> 

/* limitation: not doing [^ \\ } [:Pat_WS:]] */
<charInString> ::= [a-z] | "\\" <quoted> 

/* limitation: not doing [[\u0000-\U00010FFFF]-[uxUN]] */
<quoted> ::= ("u" (<hex> <hex> <hex> <hex> | <bracketedHex>)) | ("x" (<hex> <hex> | <bracketedHex>)) | ("U00" ("0" <hex> <hex> <hex> <hex> <hex>) | ("10" <hex> <hex> <hex> <hex>)) | ("N{" <propName> "}") | ([a-t] | [v-w] | [y-z]) 

<charName> ::= <s> ([A-Z] | [a-z] | [0-9]) ([A-Z] | [a-z] | [0-9] | "_" | " " | "-")* <s> 

<bracketedHex> ::= "{" <s> <hexCodePoint> (<s> <hexCodePoint>)* <s> "}" 

<hexCodePoint> ::= <hex>+ | ("10" <hex> <hex> <hex> <hex>) 

<hex> ::= ([0-9] | [A-F] | [a-f] ) 

/* instead of: [:ID_Start:] [:ID_Continue:]* */
<var> ::= "$" [a-z]+ 

/* instead of: [:Pattern_White_Space:]*  */
<s> ::= " "* 
-->

Some constraints on UnicodeSet syntax are not captured by this EBNF. Notably, property names and values are restricted to those supported by the implementation, and have additional constraints imposed by [[UAX44](https://www.unicode.org/reports/tr41/#UAX44)]. Quoted characters in strings must resolve to exactly one character. That is, **[\{\x{61}}]** is valid and equivalent to **[\{a}]**, but **[\{\\x{61 62}}]** is invalid and in particular not equivalent to **[\\x\{61 62}]**, which is equivalent to **[\{ab}]**.

The syntax characters are listed in the table below:

| Char | Hex    | Name                 | Usage                                      |
| ---- | ------ | -------------------- | ------------------------------------------ |
|  $   | U+0024 | DOLLAR SIGN          | Variable prefix and standin for \\uFFFF (This is for implementations that return \\uFFFF when accessing before the first or after the last character) |
...
Note that some syntax characters only have a special meaning in a certain context. In particular, out of all above syntax characters, only \\, \}, and whitespace have a special meaning inside strings, and:
* \$ is equivalent to \uFFFF when appearing at the very end of a set (**[a-z\$]**), and used as starting indicator for a variable reference elsewhere (**[\$my_set]**)
* \- is equivalent to the literal character \\- when occuring at the very beginning of a set, after a \^ at the beginning of a set, or at the very end of a set (**[-abc]**, **[^-abc]**, **[abc-]**), and used as the set difference or range operator elsewhere (**[[abc]-[bc]]**, **[a-z]**)
...
UnicodeSets may contain variables (`$my_char`, `$the_set`, ...) in place of full UnicodeSets and strings (and by extension characters).

This introduces ambiguities when parsing. For example, it is unclear whether **[$a-$b]** is a range of strings/characters, or a set difference. Thus, implementations that support variables may want to require the value of variables to be known at UnicodeSet parse time.
