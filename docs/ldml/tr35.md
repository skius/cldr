...
UnicodeSets may contain variables (`$my_char`, `$the_set`, ...) in place of full UnicodeSets and strings.

This introduces ambiguities when parsing. For example, it is unclear whether **[$a-$b]** is a range of strings (or by extensions characters), or a set difference. Thus, implementations that support variables may want to require the value of variables to be known at UnicodeSet parse time.
